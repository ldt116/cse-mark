...
| `/mark` | Đã bind | Tổng hợp điểm mọi lớp đang học của MSSV đã bind. Chỉ qua DM. |
...
- `/bind`, `/mark` (đã bind) trong group → "chỉ dùng trong nhắn riêng (DM) với bot" — identity là dữ liệu cá nhân, không chạy trong group (issue #38).
