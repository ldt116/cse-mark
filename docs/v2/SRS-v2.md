...
Hệ thống hỗ trợ admin vận hành và sinh viên trên **hai nền tảng giao tiếp** (Telegram và Discord), dùng chung một backend dữ liệu học tập:
...
Lưu: Student (roster), Binding, Verification (TTL), Class (= course), DiscordMapping, Mark cache, cấu hình. **Database không phải nguồn dữ liệu chính của điểm/roster** — chỉ là cache sau đồng bộ.
...
- Tạo / cập nhật lớp học và đồng bộ dữ liệu (`/create`, `/sync`).
- Dọn dữ liệu lớp trong backend qua `/clear` trên Telegram.
- Xem log hệ thống.
...
> v2 **không dùng ownership theo lớp học**. Mọi Admin đều nhìn và thao tác được trên toàn bộ lớp trong hệ thống. Các trường legacy như `by_id`/`by_user` nếu còn tồn tại ở tầng lưu trữ chỉ phục vụ tương thích ngược, không tham gia phân quyền.
...
Channel chỉ cho phép role tương ứng truy cập. Quyền truy cập rộng hơn của Admin Discord được cấu hình ở cấp server/guild, tách biệt khỏi class role sync.
...
- **Admin** — whitelist config (platform UserID qua env).
...
tra Discord RoleID qua DiscordMapping
        │
        ▼
discord.Bot: gán role cho enrolled, gỡ role cho học viên cũ
```
...
- Quyền Admin ở Discord được quản lý ngoài class role sync; bot không tự gán class role cho Admin chỉ để mở quyền xem kênh.
